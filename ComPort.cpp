...
namespace
{
void SleepMs( unsigned int t_out )
{
   std::this_thread::sleep_for(std::chrono::milliseconds(t_out));
}
}
// конструкторы /деструктор
COMPort::COMPort(std::string const & portName,
   std::unique_ptr<COMMTIMEOUTS>_Timeouts, unsigned int CommMask,
   unsigned int InputBuffer, unsigned int Outputbuffer)
   : port_name(portName), TimeOuts(std::move(_Timeouts)), ComDCM(),
   PortHandle(0)
{
   PortHandle = CreateFileA(portName.c_str(), GENERIC_READ | GENERIC_WRITE, 0,
...
COMPort::COMPort(const string &portName) : port_name(portName),
   TimeOuts(new COMMTIMEOUTS()), ComDCM(), PortHandle(0)
{
   PortHandle = CreateFileA(portName.c_str(), GENERIC_READ | GENERIC_WRITE, 0,
      NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
...
// настройка порта
bool COMPort::Set_DCB_Settings(BaudRates BaudRate, Paritys patity,
   fParitys f_parity, StopBits stop_bits)
{
   if (!ComDCM.get())
   {
